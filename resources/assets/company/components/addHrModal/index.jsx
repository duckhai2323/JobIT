...
import styles from './addHrModal.module.scss';
...
const AddHrModal = ({ displayModal, onClickHandle, companyId, getListHrs, loader }) => {
  ...
  const addAdmin = (e) => {
    ...
    if(password === confirmPassword && password !== "") {
      ...
      const addHr = async () => {
        loader(true);
        const response = await signUpService(inputData);
        if (response) {
          const newHr = response;
          const response2 = await createNewReference({
            user_id: newHr.id,
            company_id: companyId,
            role: "3",
          });
          if (response2.success) {
            console.log("create new Hr successful");
            getListHrs();
            loader(true);
          }
        }
      ...
      <div className={cx('modal-background')}>
        <div className={cx('modal-container')}>
          <div className={cx('modal-header')}>
            <div className={cx('tabs')}><span className={cx('modal-title')}>Thêm nhân viên mới</span></div>
